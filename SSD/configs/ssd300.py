...
train = dict(
    batch_size=16, #changing from 32 to 64
    amp=True, # Automatic mixed precision
    log_interval=20,
    seed=0,
    #epochs=50,
    #epochs=20, #changing to 20 epochs (to have 6000 iterations, 20*312 is approximately 6000)
    epochs = 32, #changing to 32 epochs for task 4c (10000 iterations / 312 = 32 epochs approximately)
...
optimizer = L(torch.optim.SGD)(
    ...
    lr=4e-3, momentum=0.9, weight_decay=0.0005 
...
data_train=dict(
    dataset=L(MNISTDetectionDataset)(
        ...
        transform=L(torchvision.transforms.Compose)(transforms=[
            ...
            L(ColorJitter)(brightness = 0.05, contrast=0.05, saturation=0.05), #applying some jitter
            # GroundTruthBoxesToAnchors assigns each ground truth to anchors, required to compute loss in training.
            L(GroundTruthBoxesToAnchors)(anchors="${anchors}", iou_threshold=0.5),
        ])
    ),
    dataloader=L(torch.utils.data.DataLoader)( #endrer num_workers til 2
