...
def read_predicted_boxes():
    json_file = read_json_file("predicted_boxes.json") #changed path so it was correct for me
...
def read_ground_truth_boxes():
    json_file = read_json_file("ground_truth_boxes.json") #changed path so it was correct for me
    for image_id in json_file.keys():
